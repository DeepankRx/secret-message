...
const SignUp = () => {
  ...
  const [buttonText,setButtonText]=useState('Create Link');
  ...
  return (
    <Card>
      {!link && (
          ...
          />
          <button className=' bg-[rgb(64,221,255)] text-white rounded-md font-semibold p-2  shadow-md  shadow-blue-400 ' onClick={createUser}>{buttonText}</button>
          </div>
      )}
      {link && (
